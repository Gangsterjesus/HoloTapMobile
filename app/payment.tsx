...
export default function Payment() {
  ...
  async function handlePay() {
    const res = await fetch("http://192.168.1.205:3000/api/payment/process", {
      method: "POST",
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.label}>Merchant: {merchantId}</Text>
      <Text style={styles.label}>Session: {sessionId}</Text>
